        ...
        default: {
          ...
          if (this.defaults.extensions?.childTokens?.[genericToken.type]) {
            this.defaults.extensions.childTokens[genericToken.type].forEach(
              (childTokens) => {
                const tokens = genericToken[childTokens].flat(Infinity) as
                  | Token[]
                  | TokensList;
  ...
  use(...args: MarkedExtension[]) {
    ...
    args.forEach((pack) => {
      ...
      const opts: MarkedOptions = {
        ...pack,
        extensions,
        hooks: pack.hooks ? Object.assign(new _Hooks(), pack.hooks) : undefined,
        renderer: pack.renderer
          ? Object.assign(new _Renderer(this.defaults), pack.renderer)
          : undefined,
        tokenizer: pack.tokenizer
          ? Object.assign(new _Tokenizer(this.defaults), pack.tokenizer)
          : undefined,
      };
